...
namespace RecruitXpress_BE.Controllers
{
    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        public AuthenticationController(RecruitXpressContext context, IConfiguration configuration,
            IEmailSender emailSender, IGoogleService googleService, IAccountRepository accountRepository)
        {
            _context = context;
        ...
        public async Task<IActionResult> ResetPasswordByEmail(string email)
        {
            try
            {
                ...
                var token = TokenHelper.GenerateRandomToken(64);
                string url = _configuration["Website:ClientUrl"] + "/set-password/" + token;
        ...
        public async Task<IActionResult> Login(LoginModel model)
        {
            ...
            var user = await _context.Accounts.Include(a => a.Role).Include(r => r.Profiles)
                .Include(c => c.CandidateCvs).SingleOrDefaultAsync(u => u.Account1 == model.Username);
        ...
        public async Task<IActionResult> GrantAccess(AccessModel model)
        {
            ...
            var user = await _context.AccessCodes.SingleOrDefaultAsync(u =>
                u.Email == model.email && u.Code.Equals(model.code));
        ...
        public async Task<IActionResult> Callback()
        {
            try
            {
                ...
                if (userInfo == null) throw new Exception("Không thể tìm thấy thông tin đăng nhập của bạn!");
                var user = await _context.Accounts.Include(a => a.Role)
                    .Include(r => r.Profiles)
                    .Include(c => c.CandidateCvs)
                    .SingleOrDefaultAsync(u => u.Account1 == userInfo.Email) ?? await _accountRepository.AddAccount(new Account()
                {
                    ...
                    RoleId = Constant.ROLE.CANDIDATE,
